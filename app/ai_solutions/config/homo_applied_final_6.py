...
lab_map_path = None
if os.path.exists('config/selected_lab_map.txt'):
    with open('config/selected_lab_map.txt', 'r') as f:
        lab_map_path = f.read().strip()
if not lab_map_path or not os.path.exists(lab_map_path):
    # Default to assets/floorplan_gui.png if not set
    lab_map_path = 'assets/floorplan_gui.png'
    if not os.path.exists(lab_map_path):
        raise FileNotFoundError("Lab map image path not found. Please run main_unified.py and select a floorplan image first.")
...
with open(f"config/homography_mappings_{camera_name}.json", "w") as f:
